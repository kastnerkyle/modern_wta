...
def make_block_causal_masks(inputs,
    ...
    def get_steps(inputs):
        ...
        last_steps += offsets
        # make this the max sequence length?
...
class PerceiverSUNMASK(nn.Module):
    ...
    def forward(self, inputs, input_idxs, input_mask, input_mem_mask):
        ...
        # latent, batch
        non_neg_latent_last_steps = (latent_last_steps * ((latent_last_steps >= 0) * 1.) + (inputs.shape[0] - 1) * ((latent_last_steps < 0) * 1.)).long()
        #latent_positions = torch.gather(positions, 0, latent_last_steps.T.long())
        latent_positions = torch.gather(positions, 0, non_neg_latent_last_steps.T.long())
        ...
        last_step_embeddings = torch.gather(embedded_inputs_query, 0, non_neg_latent_last_steps.T[..., None].repeat(1, 1, self._input_embed_dim // 2).long())
        ...
        # same for mem mask
        input_mem_mask_embed = input_mem_mask[..., None] * 1. / np.sqrt(self._input_embed_dim)
        input_mem_mask_embed = 0. * embedded_inputs_inputs + input_mem_mask_embed
        embedded_inputs_inputs = torch.concat([embedded_inputs_inputs, input_mem_mask_embed], axis=-1)
