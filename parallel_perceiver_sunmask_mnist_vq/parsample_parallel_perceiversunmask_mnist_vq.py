...
models = {}
for quad in ["A", "B", "C", "D"]:
    model_save_path = "parallel_perceiversunmask_mnist_vq_models_quad_{}".format(quad)
...
def torch_paryield_diffuse_perceiversunmask(batch_fn, model,
                                            vocabulary_size,
                                            current_buffer=None,
                                            ...
                                            verbose=True):
    # make batch here...
    batch, batch_idx, targets = batch_fn(current_buffer)
    C = 0 * targets[:, :]
    assert C.shape[1] == batch.shape[1]
    #batch = torch.tensor(batch).type(torch.FloatTensor).to(device)
    #batch_idx = torch.tensor(batch_idx).type(torch.FloatTensor).to(device)
    #targets = torch.tensor(targets).type(torch.FloatTensor).to(device)
    #C = torch.tensor(C).type(torch.FloatTensor).to(device)
    ...
    with torch.no_grad():
        ...
        for n in range(int(n_steps + n_reps_final_mask_dwell)):
            ...
            """
            C2 = torch.clone(C)
            # bidirectional communication with generator, send in updated data each time
            current_buffer = yield (x,), True
            batch, batch_idx, targets = batch_fn(current_buffer)
            x = torch.tensor(batch).type(torch.FloatTensor).to(device)
            x_idx = torch.tensor(batch_idx).type(torch.FloatTensor).to(device)
        if return_intermediates:
            yield (x, all_x, all_C), False
        yield (x,), False
...
if True:
    ...
    def make_quad_B(data, target_quadrant="B"):
        return make_batch(data, tmp_random, target_quadrant=target_quadrant)

    def make_quad_C(data, target_quadrant="C"):
        return make_batch(data, tmp_random, target_quadrant=target_quadrant)

    def make_quad_D(data, target_quadrant="D"):
        return make_batch(data, tmp_random, target_quadrant=target_quadrant)

    def batch_to_data(raw_pred, target_quadrant):
        ...
        reordered_preds = []
        for ind in range(raw_pred.shape[1]):
            if target_quadrant == "A":
            ...
            reordered_preds.append(reordered_pred)
        return np.concatenate([rp[None, None] for rp in reordered_preds], axis=0)

    # uses a set of 4 bidirectional generators to sequentiall step through sampling each quadrant, passing data to the next
    # stateful generator function
    continue_loop = True
    par_loops = 0
    batch_fns = {"A": make_quad_A,
                 "B": make_quad_B,
                 "C": make_quad_C,
                 "D": make_quad_D}
    ret_gens = {"A": None,
        ...
        for model_quad in ["A", "B", "C", "D"]:
            if par_loops == 0:
                ret_gen_Q = torch_paryield_diffuse_perceiversunmask(batch_fns[model_quad],
                                                                    ...
                                                                    verbose=False)
                ret_tup = next(ret_gen_Q)
                ret_gens[model_quad] = ret_gen_Q
            else:
                ret_tup = ret_gens[model_quad].send(data)

            not_finished = ret_tup[-1]
            if len(ret_tup[0]) == 1:
                ret_Q = ret_tup[0][0]
            else:
                print("multi-return?")
    ...
    # use 0th example
    reordered_pred = np.copy(data)[0, 0]
